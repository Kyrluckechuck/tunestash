...
- `make dev` - Start all services locally (requires local PostgreSQL, Celery, Redis/PostgreSQL broker)
- `make dev-api` - Start only the API server locally
- `make dev-frontend` - Start only the frontend dev server locally  
- `make dev-worker` - Start only the Celery worker locally

### Testing & Quality Assurance
- `make test` - Run tests for both API and frontend
- `make test-api` - Run API tests with PostgreSQL test database
- `make test-frontend` - Run frontend tests using Vitest  
- `make lint` - Run linting for both API and frontend
- `make lint-api` - Run comprehensive API linting (flake8, black, isort, mypy, bandit, pylint)
- `make lint-frontend` - Run frontend linting using ESLint
- `make format` - Format code for both API and frontend
...
## Development Setup (Docker-First)
...
- Run tests: `make test-api` or `python -m pytest` from root
...
4. Test with `yarn test:graphql`

### Database Changes
1. Modify models in `api/library_manager/models.py`
2. Create migration: `cd api && python manage.py makemigrations`
3. Apply migration: `make migrate`
4. Update any affected GraphQL types
