...
# Specific files to ignore (relative paths from repo root)
IGNORE_FILES = {
    'frontend/src/types/generated/graphql.ts',  # Auto-generated by graphql-codegen
}

def should_check_file(file_path: Path, repo_root: Path = None) -> bool:
    """Check if a file should be checked for trailing newlines."""
    # Check if it's in the specific ignore list
    if repo_root:
        try:
            ...
            pass  # file_path is not relative to repo_root
...
def get_staged_files(repo_root: Path) -> List[Path]:
    ...
    for rel_path in staged_paths:
        ...
        if candidate.is_file() and should_check_file(candidate, repo_root):
